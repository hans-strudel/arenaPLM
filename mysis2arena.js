...
function loadData(){
	...
	for (i= 2; i <= rows; i++){
		...
		//	boms.push({"name":obj.name, "Fname":obj.Fname, "rev":obj.rev, "desc":obj.desc})
			...
			// "rev":obj.rev,
			// "nrev":revs[compare.indexOf(obj.Fname)]||revs[compare.indexOf(obj.name)], "desc":obj.desc})
	...
	boms.forEach(function(elem,ind,arr){
		output = '0,' + (elem.Fname || elem.name) + ',"' + elem.desc + '",' + elem.rev + '-A,1,X,0\r\n'
	...
	try {
		fs.unlinkSync('boms\\' + (elem.name) + ' rev ' + (elem.rev) + '.csv')
	} catch (e){
		console.log(e)
	}
		fs.writeFile('boms\\' + (elem.name) + ' rev ' + (elem.rev) + '.csv', 
